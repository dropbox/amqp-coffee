...
declare module 'event-to-promise' {
  ...
  namespace eventToPromise {
    /*
  	 * Wait for one of multiple events. The array of all the parameters of the
  	 * emitted event is used to resolve/reject the promise.
  	 *
  	 ...
  	 * @return               The returned promise has a `cancel()` method which can be used to
  	 *   remove the event listeners. Note that the promise will never settled if canceled.
  	 */
