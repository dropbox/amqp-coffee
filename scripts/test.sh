...
if $COVERAGE; then
  ...
  # AMQP_TEST=1 NODE_PATH=$ROOT/bin-cov $MOCHA --require 'coffee-script/register' --compilers coffee:coffee-script --reporter html-cov --ui bdd --grep "$GREP" $TESTS > $ROOT/coverage.html
